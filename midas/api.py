...
remote_large_model_path = "https://github.com/intel-isl/DPT/releases/download/1_0/dpt_large-midas-2f21e586.pt"
...
def load_model(model_type, device="cpu"):
    ...
    if model_type == "dpt_large":  # DPT-Large
        if os.path.exists(old_model_path):
            ...
            load_file_from_url(remote_large_model_path, model_dir=base_model_path)
        model = DPTDepthModel(
            path=model_path,
            backbone="vitl16_384",
            non_negative=True,
...
class MiDaSInference(nn.Module):
    ...
    def __init__(self, model_type, device='cpu'):
        ...
        model, _ = load_model(model_type, device=device)
